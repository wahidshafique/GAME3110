...
//	server->AttachPlugin(&packetLogger);
	...
	const unsigned int maxConnections = 8;
	bool isSuccess = server->Startup(maxConnections, socketDescriptors, 2 )==RakNet::RAKNET_STARTED;
	...
	if (!isSuccess)
	{
		...
		isSuccess = server->Startup(4, socketDescriptors, 1 )==RakNet::RAKNET_STARTED;
		if (!isSuccess)
		{
